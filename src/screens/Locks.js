...
export default function BikesManagement() {
  ...
  // Thêm bike với kiểm tra lock tồn tại và chưa được dùng
  const handleAddBike = () => {
    if (!newBike.bikeId) {
    ...
    if (lockOptions.length === 0) {
      alert("Không còn lock trống. Vui lòng thêm lock mới trước khi thêm bike.");
      return;
    }
    if (!lockOptions.includes(newBike.currentLockId)) {
      alert(
        `Lock ID ${newBike.currentLockId} đã được sử dụng hoặc không tồn tại. Vui lòng chọn lock khác.`
      );
      return;
    }
  ...
  const handleView = (bike) => {
    alert(
      `Details:\nBike ID: ${bike.bikeId}\nStatus: ${bike.status}\nLock ID: ${bike.currentLockId}\nUser ID: ${bike.currentUserId}`
    );
  };
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                >
                  <option value="">Chọn lock còn trống</option>
                  {lockOptions.length > 0 ? (
                    ...
                    <option disabled>-- Hết lock trống, vui lòng thêm lock --</option>
                  )}
                </select>
              </div>
