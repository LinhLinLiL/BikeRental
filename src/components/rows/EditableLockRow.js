// 📁 src/components/rows/EditableLockRow.jsx
...
export default function EditableLockRow({ lock }) {
  ...
  const handleSave = async () => {
    try {
      ...
      await update(lockRef, editedLock);
      setEditing(false);
    } catch (err) {
  ...
  const handleDelete = async () => {
    if (!window.confirm(`Xác nhận xoá khoá ${lock.id}?`)) return;
    try {
      const lockRef = ref(db, `locks/${lock.id}`);
      ...
      {editing ? (
        <>
          <td className="py-2 px-4">
            <input className="border p-1 w-full" value={editedLock.id} onChange={(e) => setEditedLock({ ...editedLock, id: e.target.value })} />
          </td>
          <td className="py-2 px-4">
            <input className="border p-1 w-full" value={editedLock.bikeId || ""} onChange={(e) => setEditedLock({ ...editedLock, bikeId: e.target.value })} />
          </td>
          <td className="py-2 px-4">
            <select className="border p-1 w-full" value={editedLock.occupied} onChange={(e) => setEditedLock({ ...editedLock, occupied: e.target.value === "true" })}>
              <option value="true">Yes</option>
              <option value="false">No</option>
            </select>
          </td>
          <td className="py-2 px-4">
            <input className="border p-1 w-full" value={editedLock.otp || ""} onChange={(e) => setEditedLock({ ...editedLock, otp: e.target.value })} />
          </td>
          <td className="py-2 px-4">{formatTime(editedLock.otpTimestamp)}</td>
          <td className="py-2 px-4">
            <select className="border p-1 w-full" value={editedLock.isValid} onChange={(e) => setEditedLock({ ...editedLock, isValid: e.target.value === "true" })}>
              <option value="true">✔</option>
              <option value="false">✖</option>
            </select>
          </td>
          <td className="py-2 px-4">
            <input className="border p-1 w-full" value={editedLock.returnBikeId || ""} onChange={(e) => setEditedLock({ ...editedLock, returnBikeId: e.target.value })} />
          </td>
